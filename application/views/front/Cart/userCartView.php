...
<?php 
    $totalAmt = 0;
    foreach ($cartData as $cartKey => $cartVal) {
            if(count($cartVal) > 0)
            {
                if(isset($cartVal['cart_data']['productData']))
                { ?>
                    ...
                    <?php 
                    if(isset($cartVal['cart_data']['productOptions']))
                    {
                        $productKeys = array_keys($cartVal['cart_data']['productOptions']);
                        $lastKey = end($productKeys);
                        foreach($cartVal['cart_data']['productOptions'] as $optKey => $optVal)
                        { ?>
                 ...
                 <?php } ?>
                    ...
                    <h4 class="fs-md ft-medium mb-0 lh-1"><?php echo $cartVal['cart_data']['productData']['price']; ?></h4></div></div>
                    <div class="fls_last"><button class="close_slide gray" onclick="deleteCartItem(<?php echo $cartVal['cart_data']['productData']['cartId']; ?>)"><i class="ti-close"></i></button></div></div>
                <?php 
